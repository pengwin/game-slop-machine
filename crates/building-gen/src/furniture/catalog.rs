...
use crate::mesh::math_util::{append_colored_quad, append_colored_triangle, append_quad, Quad};
    ...
    for (i, &(tx, ty)) in center_positions.iter().enumerate().take(4) {
        ...
        if i % 2 == 0 {
            items.push(FurnitureItem {
                ...
                height: 0.6,
                depth: 0.4,
                color: [1.0, 1.0, 1.0], // Use white so vertex colors show properly
...
pub fn single_item(item_type: FurnitureType) -> FurnitureItem {
    ...
    let (w, h, d, color, mesh) = match item_type {
        ...
        FurnitureType::Barrel => {
            let (d, h) = (0.4, 0.6);
            (d, h, d, [1.0, 1.0, 1.0], generate_barrel_mesh(d, h, &BarrelConfig::default()))
        }
        FurnitureType::Crate => {
